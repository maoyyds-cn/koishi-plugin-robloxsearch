...
| 项目 | 要求 | 说明 |
| --- | --- | --- |
| Node.js | `>= 18`（含 npm） | 运行 `node -v` / `npm -v` 确认版本 |
| 服务器 | 可公网访问（或有内网穿透 / 反向代理出口） | 图片需从 QQ 端可访问，仅本机监听无法对外提供服务 |
| 公网地址 / 域名 | 一个对外可访问的 `http(s)` 地址 | 必填，用于配置 `PUBLIC_BASE_URL`（拼接返回的 `localUrl`） |
| 可写目录 | 用于存放转存后的图片 | 服务启动时按需创建 `uploads/`，确保运行用户有写权限 |
| （可选）HTTPS 与鉴权 | 建议公网部署开启 | 通过 Nginx 反代启用 HTTPS，并设置 `AUTH_TOKEN` |
